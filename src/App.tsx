...
const App = () => {
  ...
  const getFinanceStats = (barbeiroNome?: string) => {
    ...
    const hoje = registros
      .filter((h) => h.dataConclusao?.toMillis() >= hojeStart)
      .reduce((acc, h) => acc + (h.valor || 0), 0);
    const semana = registros
      .filter((h) => h.dataConclusao?.toMillis() >= semanaStart)
      .reduce((acc, h) => acc + (h.valor || 0), 0);
    const mes = registros
      .filter((h) => h.dataConclusao?.toMillis() >= mesStart)
      .reduce((acc, h) => acc + (h.valor || 0), 0);
  ...
  const confirmarFinalizacao = async () => {
    ...
    try {
      ...
      addToast("Erro ao processar checkout.", "erro");
    } finally {
      setLoadingAction(null);
    }
  };
          ...
          <div className="flex justify-between items-center border-b border-white/5 pb-8">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="px-6 py-4 bg-blue-600/10 border border-blue-500/20 text-blue-400 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-600 hover:text-white transition-all flex items-center gap-2"
              >
              ...
              <GlassContainer className="w-full max-w-lg space-y-8 border-blue-500/20">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="p-6 bg-slate-950 rounded-3xl border border-white/5 flex justify-between items-center">
                    <span className="font-black uppercase text-xs text-slate-500 tracking-widest">
                      Hoje
                    </span>
                    <span className="text-2xl font-black text-white">
                      {formatCurrency(meusGanhos.hoje)}
                    </span>
                  </div>
                  <div className="p-6 bg-slate-950 rounded-3xl border border-white/5 flex justify-between items-center">
                    <span className="font-black uppercase text-xs text-slate-500 tracking-widest">
                    ...
                    <span className="text-2xl font-black text-white">
                      {formatCurrency(meusGanhos.semana)}
                    </span>
                  </div>
                  <div className="p-6 bg-slate-950 rounded-3xl border border-blue-500/20 flex justify-between items-center">
                    <span className="font-black uppercase text-xs text-blue-400 tracking-widest">
                      Mês
                    </span>
                    <span className="text-2xl font-black text-blue-400">
                      {formatCurrency(meusGanhos.mes)}
                    </span>
                  </div>
                </div>
                <button
              ...
              <GlassContainer className="w-full max-w-md space-y-10 border-emerald-500/30">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">
                    Cliente: {checkoutAtivo.nome}
                  </p>
                </div>
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase text-slate-600 text-center block tracking-[0.2em]">
                    Valor do Serviço (R$)
                  </label>
                  <input
                    type="number"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    className="p-6 bg-emerald-600 rounded-3xl font-black uppercase text-xs shadow-xl shadow-emerald-900/20"
                  >
                    Confirmar
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <GlassContainer className="bg-slate-900/30 border-emerald-500/20">
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className="font-black uppercase text-[10px] tracking-widest text-slate-500">
                  💰 Hoje
                </span>
              </div>
