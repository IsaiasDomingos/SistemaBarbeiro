...
const App = () => {
  ...
  // 🎙️ CARREGADOR DE VOZES E BANCO DE DADOS
  useEffect(() => {
  ...
  // 🗣️ FUNÇÃO DA VOZ DA TV COM CORREÇÃO DE ENGASGO
  const falarAnuncio = (nomeCliente, nomeBarbeiro, numCadeira) => {
    if (!window.speechSynthesis) return;
    const cadeiraTexto = numCadeira ? `na cadeira ${numCadeira}` : "";
    const texto = `${nomeCliente}. Chegou a sua vez com ${nomeBarbeiro} ${cadeiraTexto}.`;
  ...
  const getAdvancedStats = () => {
    ...
    historicoAtendimentos.forEach((h) => {
      ...
      if (
        time >= lastQuarterStart.getTime() &&
        time < thisQuarterStart.getTime()
      )
        histLastQuarter.push(h);
    });
    ...
    return {
      ...
      trimestreClient:
        topClientName !== "NENHUM NO PERÍODO"
          ? `${topClientName} (${topClientCount} cortes)`
          : "Nenhum no período",
    };
  };
  ...
  const limparHistoricoCompleto = async () => {
    if (!confirm("Deseja apagar TODO o histórico? Esta ação é permanente!"))
      return;
    try {
  ...
  // ================= TELA 1: CADASTRO COM MÚLTIPLAS ESCOLHAS ATUALIZADO =================
        ...
        <GlassContainer className="w-full max-w-lg space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase text-slate-500 text-center block tracking-widest">
              SELECIONE OS SERVIÇOS (PODE SER MAIS DE UM)
            </label>
            <div className="grid grid-cols-2 gap-3">
              {/* REMOVIDO "COMPLETO" E ADICIONADO "OUTROS" COM LOGICA PURA DE MULTIPLA ESCOLHA */}
              {["CABELO", "BARBA", "SOBRANCELHA", "OUTROS"].map((s) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (servs.includes(s)) {
                      servs = servs.filter((x) => x !== s); // Desmarca se já tem
                    } else {
                      servs.push(s); // Marca se não tem
                    }
                    if (servs.length === 0) servs = ["CABELO"]; // Trava de segurança para não enviar vazio
    ...
    return (
      <div className="h-screen w-screen overflow-hidden bg-slate-950 p-6 text-white flex flex-col">
        <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-4 shrink-0">
          <button
            onClick={() => setModo("selecao")}
            className="bg-slate-900 p-3 rounded-2xl text-slate-500"
          >
            <ArrowLeft size={24} />
          </button>
          <div className="text-center">
            <h1 className="text-4xl font-black uppercase italic tracking-widest neon-yellow">
              ELITE CARIOCA
            </h1>
            <p className="text-blue-500 font-bold tracking-[0.6em] text-[8px] uppercase neon-blue">
        ...
        >
          <div className="glass rounded-[2rem] p-4 h-full flex flex-col border border-blue-500/20 bg-slate-900/30 overflow-hidden">
            <h3 className="text-center font-black text-blue-400 uppercase text-[10px] tracking-widest mb-3 border-b border-white/5 pb-2 shrink-0">
              FILA GERAL -{" "}
              <span className="text-white text-sm">{filaGeralCount}</span>
            </h3>
            <div className="flex flex-col gap-2 flex-1 justify-start overflow-hidden">
              ...
              {filaGeralDisplay.map((c) => (
                <div
                  key={c.id}
                  className="p-3 bg-slate-800 rounded-xl flex flex-col items-center justify-center border border-white/5 shrink-0 relative pt-5"
                >
                  ...
                  <span className="text-lg font-black block uppercase leading-none mb-1 truncate w-full text-center px-2">
                    {c.nome}
                  </span>
            ...
            return (
              ...
              >
                <h3 className="text-center font-black text-yellow-400 uppercase text-[10px] tracking-widest mb-3 border-b border-white/5 pb-2 shrink-0">
                  ...
                  {p.cadeira} -{" "}
                  <span className="text-white text-sm">{countWaiting}</span>
                  <div
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border flex flex-col items-center justify-center transition-all relative ${
                        c.isAtendimento
                          ...
                          : "bg-slate-800/50 border-white/5 shrink-0 pt-5"
                      }`}
                    >
                      {c.isAtendimento ? (
        ...
        <div className="w-full max-w-5xl space-y-10">
          <div className="flex justify-between items-center border-b border-white/5 pb-8 pt-4">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h2 className="text-4xl font-black uppercase tracking-tighter neon-yellow leading-none">
                  {barbeiroLogado.nome}
                </h2>
            ...
            <button
              ...
              className="p-5 bg-slate-900 rounded-3xl text-red-500 hover:bg-red-500 hover:text-white transition-all"
            >
              <LogOut size={24} />
            </button>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="font-black text-xs text-slate-500 uppercase tracking-widest px-2">
              💰 Faturamento
            </h3>
            ...
            <h3 className="font-black text-xs text-slate-500 uppercase tracking-widest px-2 mt-8">
              📊 Meu Desempenho (Mês Atual)
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-5 bg-slate-900/30 rounded-2xl border border-white/5 flex items-center justify-between">
                <div>
              ...
              <button
                ...
                className={`p-6 rounded-[2rem] font-black uppercase text-xs border-2 transition-all ${
                  dAtual.status === st
                    ...
                    : "bg-slate-900 border-white/5 opacity-50 hover:opacity-100"
                }`}
              >
          ...
          <div className="grid grid-cols-1 gap-12">
            {emAtend.length > 0 ? (
              <div className="p-10 bg-slate-900 rounded-[3rem] border-2 border-emerald-500/30 text-center space-y-6">
                ...
                <h4 className="text-6xl font-black uppercase text-white leading-none">
                  {emAtend[0].nome}
                </h4>
                <div className="py-2">
                ...
                <button
                  ...
                  className="w-full bg-emerald-600 p-8 rounded-3xl font-black text-xl uppercase hover:bg-emerald-500 transition-all shadow-xl shadow-emerald-900/20 mt-4"
                >
                  FINALIZAR & RECEBER
                </button>
              </div>
  ...
  if (modo === "gestao_master") {
    ...
    const salvarProfissional = async () => {
      if (!novoProf.nome || !novoProf.matricula)
        return addToast("Preencha Nome e Matrícula", "erro");
      if (profEditando) {
        await db
          .collection("profissionais")
          .doc(profEditando)
      ...
      setNovoProf({ nome: "", matricula: "", cadeira: "" });
      ...
      <div className="min-h-screen bg-slate-950 p-8 text-white flex flex-col items-center overflow-y-auto custom-scrollbar">
        <div className="w-full max-w-6xl space-y-10 mb-20">
          <div className="flex justify-between items-center">
            <button
              onClick={() => setModo("selecao")}
              className="bg-slate-900 px-6 py-4 rounded-3xl font-black uppercase text-[10px] tracking-widest border border-white/5 hover:bg-white/5 transition-all"
            >
              <ArrowLeft size={16} /> Sair
            </button>
            <h3 className="text-2xl font-black uppercase text-yellow-500 italic flex items-center gap-2">
              <Crown size={24} /> GESTÃO MASTER
            </h3>
          ...
          <GlassContainer className="w-full space-y-6">
            <h3 className="font-black uppercase tracking-tighter text-2xl mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-8 bg-gradient-to-br from-yellow-600/20 to-yellow-900/10 rounded-3xl border border-yellow-500/30">
                <h4 className="text-yellow-500 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2">
                  <Star size={14} /> BARBEIRO ELITE ({advStats.mesPassadoNome})
                </h4>
              ...
              <div className="p-8 bg-gradient-to-br from-blue-600/20 to-blue-900/10 rounded-3xl border border-blue-500/30">
                <h4 className="text-blue-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2">
                  <Users size={14} /> CLIENTE ELITE (Trimestre Passado)
                </h4>
                <p className="text-3xl font-black uppercase text-white">
                  {advStats.trimestreClient}
                </p>
              </div>
            </div>

            <div className="pt-6">
              <h4 className="font-black uppercase text-sm mb-4 text-slate-500 tracking-widest">
                RANKING DE {advStats.mesAtualNome.toUpperCase()}
              </h4>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4 bg-slate-900/20 p-6 rounded-[2rem] border border-white/5">
                ...
                <input
                  type="text"
                  placeholder="NOME"
                  className="w-full p-4 bg-slate-950 rounded-xl border border-white/5 outline-none focus:border-yellow-500"
                  value={novoProf.nome}
                  onChange={(e) =>
                ...
                >
                  {profEditando ? "SALVAR ALTERAÇÕES" : "CADASTRAR BARBEIRO"}
                </button>
              </div>

              {/* PAINEL DE CONFIGURAÇÃO DE VOZ COM CORREÇÃO DE ENGASGO */}
              <div className="space-y-4 bg-slate-900/20 p-6 rounded-[2rem] border border-white/5">
                <h4 className="text-blue-400 font-black uppercase text-xs flex items-center gap-2">
                  <Mic size={16} /> VOZ DO PAINEL DA TV
                </h4>
                <select
                ...
                >
                  <option value="">Automática (Padrão do Aparelho)</option>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      window.speechSynthesis.cancel(); // 🚨 CORREÇÃO: Destrava a fila de áudio
                      const msg = new SpeechSynthesisUtterance(
                        "Atenção cliente, dirija-se a cadeira 1"
                      );
                      msg.lang = "pt-BR";
                      if (vozSelecionadaUI) {
                  ...
                  <button
                    onClick={async () => {
                      await db
                        .collection("configuracoes_paiva")
                        .doc("geral")
                        .set({ vozTV: vozSelecionadaUI }, { merge: true });
                ...
                <p className="text-[10px] text-slate-500">
                  Atenção: A voz salva aqui será usada na TV. Se a TV for de um
                  modelo diferente, ela pode tentar adaptar para a voz mais
                  parecida disponível nela.
                </p>
              </div>
            </div>
                ...
                {sortedProfsMaster.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          const link = `${window.location.origin}?barbeiro=${p.matricula}`;
                          navigator.clipboard.writeText(link);
                          addToast("Link copiado!", "sucesso");
                        }}
                        className="p-2.5 bg-slate-800 rounded-lg text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all"
                        title="Copiar Link"
                      >
                        <Link size={16} />
                      </button>
                      ...
                      <button
                        onClick={() => {
                          if (
                            window.confirm(`Deseja demitir/excluir ${p.nome}?`)
                          ) {
          ...
          <GlassContainer className="w-full space-y-6">
            ...
            >
              <h3 className="font-black uppercase tracking-tighter text-2xl flex items-center gap-3 select-none">
                ...
                Detalhado{" "}
                <span className="text-[10px] bg-slate-900 px-3 py-1 rounded-full text-slate-500 ml-4 group-hover:text-white transition-all">
                  {showHistorico
                    ? "CLIQUE PARA OCULTAR"
                    : "CLIQUE PARA MOSTRAR"}
                </span>
              ...
              {showHistorico && (
                ...
                >
                  <Trash2 size={14} /> Apagar Banco de Dados
                </button>
              )}
            </div>
            {showHistorico && (
