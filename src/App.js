...
import {
  ...
  AlertCircle, DollarSign, Banknote, TrendingUp, Camera, UserPlus, Link, Edit2, Star, Heart
} from "lucide-react";
...
const App = () => {
  ...
  const [novoCliente, setNovoCliente] = useState({
    ...
    sobrenome: "",
    whatsapp: "",
    servico: ["CABELO"], // MUDADO: Agora é um array para suportar Múltiplas Escolhas
  });
  ...
  const cadastrarCliente = async (barbeiro) => {
    // Transforma o array de multiplas escolhas em String antes de salvar
  ...
  // ================= TELA 1: CADASTRO COM MÚLTIPLAS ESCOLHAS =================
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-white">
        <GlassContainer className="w-full max-w-lg space-y-6">
          ...
          <div className="space-y-4">
            ...
            <input type="text" placeholder="WHATSAPP" className="w-full p-5 bg-slate-950 rounded-2xl border border-white/5 text-white font-bold outline-none focus:border-yellow-500" value={novoCliente.whatsapp} onChange={(e) => setNovoCliente({ ...novoCliente, whatsapp: e.target.value })} />
          </div>
              ...
              {["CABELO", "BARBA", "SOBRANCELHA", "COMPLETO"].map((s) => (
                <button 
                  ...
                  onClick={() => {
                    ...
                    if (s === "COMPLETO") servs = ["COMPLETO"];
                    else {
                        servs = servs.filter(x => x !== "COMPLETO");
                        if (servs.includes(s)) servs = servs.filter(x => x !== s);
                        else servs.push(s);
                    }
                    if (servs.length === 0) servs = ["CABELO"]; // Trava para não ficar vazio
  ...
  // ================= TELA 4: PAINEL DA TV (NOVO LAYOUT SEM ROLAGEM) =================
  if (modo === "painel") {
      ...
      const filaGeralDisplay = filaGeralEspera.slice(0, 3); // Mostra até 3 nomes
      
      // Cálculo de colunas e linhas dinâmico para não ter rolagem
      const totalCards = profsAtivos.length + 1; // +1 pela Fila Geral
    ...
    return (
      <div className="h-screen w-screen overflow-hidden bg-slate-950 p-6 text-white flex flex-col">
        ...
        {/* CONTAINER GRID INTELIGENTE (SEM ROLAGEM) */}
        <div 
        ...
        >
            ...
            <div className="glass rounded-[2rem] p-4 h-full flex flex-col border border-blue-500/20 bg-slate-900/30 overflow-hidden">
                ...
                <div className="flex flex-col gap-2 flex-1 justify-start overflow-hidden">
                    ...
                    {filaGeralDisplay.map(c => (
                        <div key={c.id} className="p-3 bg-slate-800 rounded-xl text-center border border-white/5 shrink-0">
                            <span className="text-lg font-black block uppercase leading-none mb-1">{c.nome}</span>
            ...
            {/* COLUNAS DOS BARBEIROS (Com destaque Em Atendimento) */}
            {profsAtivos.map(p => {
                const esperandoProf = clientesFila.filter(c => c.barbeiroPref === p.nome && c.status === "esperando");
                const countWaiting = esperandoProf.length; // Conta APENAS os que estão aguardando
                const emAtendimentoProf = clientesFila.find(c => c.barbeiroPref === p.nome && c.status === "atendendo");
                
                // Monta lista: Atendimento + 2 Esperando, ou 3 Esperando.
                let displayList = [];
                ...
                return (
                    <div key={p.id} className="glass rounded-[2rem] p-4 flex flex-col border border-yellow-500/20 bg-slate-900/30 overflow-hidden">
                        ...
                        <div className="flex flex-col gap-2 flex-1 overflow-hidden">
                            ...
                            {displayList.map((c) => (
                                <div key={c.id} className={`p-3 rounded-xl text-center border flex flex-col justify-center transition-all ${c.isAtendimento ? 'bg-emerald-600/20 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)] flex-1' : 'bg-slate-800/50 border-white/5 shrink-0'}`}>
                                    {c.isAtendimento && <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest block mb-1 flex items-center justify-center gap-1"><div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"/> EM ATENDIMENTO</span>}
                                    <span className={`font-black block uppercase leading-none mb-1 ${c.isAtendimento ? 'text-2xl text-white' : 'text-base text-slate-300'}`}>{c.nome}</span>
                                    <ServiceBadge s={c.servico} />
                                </div>
                            ))}
                        </div>
          ...
          <div className="grid grid-cols-1 gap-12">
            {emAtend.length > 0 ? (
              <div className="p-10 bg-slate-900 rounded-[3rem] border-2 border-emerald-500/30 text-center space-y-6">
                ...
                <h4 className="text-6xl font-black uppercase text-white leading-none">{emAtend[0].nome}</h4>
              ...
              <div className="p-10 bg-slate-900/50 rounded-[3rem] border border-white/5 text-center space-y-6">
                ...
                {prox ? (
                  <div className="space-y-6">
                    <h4 className="text-5xl font-black uppercase text-white">{prox.nome}</h4>
            ...
            {showHistorico && (
              <div className="overflow-x-auto pt-6 border-t border-white/5">
                <table className="w-full text-left">
                  <thead className="text-[10px] uppercase font-black text-slate-700 border-b border-white/5"><tr><th className="pb-6 px-4">DATA</th><th className="pb-6 px-4">Cliente</th><th className="pb-6 px-4 text-center">Barbeiro</th><th className="pb-6 px-4 text-center">Valor</th><th className="pb-6 px-4 text-right">Hora</th></tr></thead>
