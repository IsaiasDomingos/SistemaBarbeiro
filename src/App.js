...
import {
  ...
  AlertCircle, DollarSign, Banknote, TrendingUp, Camera, UserPlus, Link, Edit2, Star, Heart, Mic, MessageCircle, RefreshCw, Trophy, Gift, Award, Volume2, Search, Mountain, ChevronDown, ChevronUp
} from "lucide-react";
...
const BarberPole = ({ className = "" }) => (
  <div className={`w-8 md:w-12 flex flex-col items-center pt-2 pb-4 shrink-0 ${className}`}>
     ...
     <div className="flex-1 w-4 md:w-8 rounded-b-lg border-2 border-slate-700 shadow-xl overflow-hidden relative min-h-[100px]">
...
const EliteHeader = ({ subtitle, className = "" }) => (
    <div className={`flex items-stretch justify-center relative shrink-0 w-full overflow-hidden ${className}`}>
       <BarberPole className="mx-2 md:mx-6" />
       <div className="flex-1 flex flex-col items-center justify-between relative z-10 py-4 max-w-4xl">
           ...
           {/* Subtítulo forçado para alinhar com o final do poste */}
           <div className="relative z-10 w-full flex justify-center mt-6 pb-2">
...
const App = () => {
  ...
  const [isIsolado, setIsIsolado] = useState(false); // Link seguro do barbeiro
  ...
  const getAdvancedStats = () => {
    ...
    // Faturamento e Barbeiros Sazonais
    let lucroTri = 0, bTriWinner = "Nenhum";
    if (showTri) {
        const map = groupByBarber(histTri); let best = -1;
        Object.keys(map).forEach(b => {
            const d = map[b]; lucroTri += d.lucro;
            const score = d.lucro + (d.count * 10) + (d.fieis * 20) - (d.duracaoTotal ? d.duracaoTotal / d.count : 30);
            if (score > best) { best = score; bTriWinner = b; }
        });
    }

    let lucroSem = 0, bSemWinner = "Nenhum";
    if (showSem) {
        const map = groupByBarber(histSem); let best = -1;
        Object.keys(map).forEach(b => {
            const d = map[b]; lucroSem += d.lucro;
            const score = d.lucro + (d.count * 10) + (d.fieis * 20) - (d.duracaoTotal ? d.duracaoTotal / d.count : 30);
            if (score > best) { best = score; bSemWinner = b; }
        });
    }

    let lucroAno = 0, bAnoWinner = "Nenhum";
    if (showAno) {
        const map = groupByBarber(histAno); let best = -1;
        Object.keys(map).forEach(b => {
            const d = map[b]; lucroAno += d.lucro;
            const score = d.lucro + (d.count * 10) + (d.fieis * 20) - (d.duracaoTotal ? d.duracaoTotal / d.count : 30);
            if (score > best) { best = score; bAnoWinner = b; }
        });
    }
    ...
    return {
      ...
      showTri, labelTri, clientTri: calcClient(histTri), lucroTri, bTriWinner,
      showSem, labelSem, clientSem: calcClient(histSem), lucroSem, bSemWinner,
      showAno, labelAno, clientAno: calcClient(histAno), lucroAno, bAnoWinner,
    };
  };
  ...
  // LOGICA DO LINK BLINDADO (ISOLADO) DO BARBEIRO
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const matriculaUrl = params.get("barbeiro");
    
    if (matriculaUrl && profissionais.length > 0 && !barbeiroLogado && modo !== "bloqueado") {
      ...
      if (prof) { 
        ...
        // Se já está logado via link, verifica se ele foi apagado pelo dono
  ...
  }, [profissionais, modo, barbeiroLogado, isIsolado]);
  ...
  const handleLogin = () => {
    ...
    else { const prof = profissionais.find((p) => p.matricula === acessoInput); if (prof) { setBarbeiroLogado(prof); setModo("admin_barbeiro"); setIsIsolado(false); setAcessoInput(""); } else addToast("Acesso Negado", "erro"); }
  };
  ...
  const handleBuscaCpf = async () => {
      ...
      } else { addToast("CPF não encontrado. Faça seu cadastro!", "erro"); setNovoCliente({ ...novoCliente, cpf: limpo }); setModo("cliente_registro_novo"); }
  };
  ...
  const prosseguirServicos = async () => {
      ...
      if (!isBypass) {
          ...
          if (snap.exists) {
              return addToast("Este CPF já está cadastrado! Use a opção 'JÁ TENHO CADASTRO'.", "erro");
          }
  ...
  const enviarParaFila = async (barbeiro) => {
    ...
    setModo("selecao"); setNovoCliente({ nome: "", cpf: "", whatsapp: "", servico: ["CABELO"] }); setLoginCpf("");
  };
  ...
  const limparHistoricoCompleto = async () => {
    const pin = window.prompt("PERIGO: PARA APAGAR TODO O HISTÓRICO, DIGITE O PIN MASTER:");
    if (pin !== "123456") return addToast("PIN Incorreto.", "erro");
    try {
  ...
  const ServiceBadge = ({ s }) => (
    <span className="px-2 py-0.5 rounded-full border border-blue-500/20 bg-blue-500/10 text-blue-400 text-[8px] font-black uppercase tracking-widest inline-block mt-1">{s}</span>
  );

  // ================= TELAS =================

  // TELA DE BLOQUEIO (BARBEIRO DEMITIDO)
  if (modo === "bloqueado") {
      return (
  ...
  if (modo === "painel") {
    ...
    return (
      <div className="h-screen w-screen overflow-hidden bg-slate-950 text-white flex flex-col relative">
        {!audioDestravado && (
            <div onClick={() => { if (window.speechSynthesis) { window.speechSynthesis.cancel(); const msg = new SpeechSynthesisUtterance(""); window.speechSynthesis.speak(msg); } if (audioRef.current) audioRef.current.play().catch(() => {}); setAudioDestravado(true); }} className="absolute inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center cursor-pointer">
                <div className="bg-blue-600 p-10 rounded-[3rem] shadow-2xl animate-bounce flex flex-col items-center gap-4 border-4 border-white/20"><Volume2 size={64} className="text-white" /><span className="text-3xl font-black uppercase tracking-widest text-white text-center">CLIQUE AQUI PARA ATIVAR O SOM</span><span className="text-sm font-bold text-blue-200 uppercase tracking-widest text-center">NECESSÁRIO PARA SMART TV</span></div>
            </div>
        )}

        <div className="flex items-start relative z-20 w-full shrink-0 border-b border-white/5">
             <div className="flex flex-col items-start gap-3 absolute left-4 md:left-8 top-6 z-30">
                ...
                <div className="bg-blue-600/20 border border-blue-500/30 px-4 py-2 rounded-xl text-center shadow-lg shadow-blue-900/20 hidden md:block">
                    <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest block mb-1">Total em Espera</span>
                    <span className="text-2xl font-black text-white leading-none">{totalGeralEspera} clientes</span>
                </div>
            </div>
            ...
            <div className="text-right flex flex-col items-end justify-start absolute right-4 md:right-8 top-6 z-30 hidden md:flex">
                ...
                <span className="text-3xl font-black font-mono mt-1">{new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span>
            </div>
        </div>
        ...
        <div className="flex-1 flex p-4 min-w-0 overflow-hidden relative z-10 w-full">
            <div className="grid gap-4 flex-1 w-full h-full min-h-0" style={{ gridTemplateColumns: `repeat(${numCols}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${numRows}, minmax(0, 1fr))` }}>
                <div className="glass rounded-[2rem] p-4 h-full flex flex-col border border-blue-500/20 bg-slate-900/30 overflow-hidden">
                    <h3 className="text-center font-black text-blue-400 uppercase text-xs md:text-sm tracking-widest mb-3 border-b border-white/5 pb-2 shrink-0">FILA GERAL - <span className="text-white text-lg ml-1">{filaGeralCount}</span></h3>
                    <div className="flex flex-col gap-2 flex-1 justify-start overflow-hidden">
                        ...
                        {filaGeralDisplay.map(c => (
                            <div key={c.id} className="p-3 md:p-4 bg-slate-800 rounded-xl flex flex-col items-center justify-center border border-white/5 shrink-0 relative pt-6 md:pt-8"><span className="absolute top-2 right-2 text-[10px] text-slate-400 font-mono font-bold bg-slate-950/50 px-2 py-1 rounded">{c.chegada?.toDate().toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit' })}</span><span className="text-xl md:text-2xl font-black block uppercase leading-none mb-1 truncate w-full text-center px-2">{formatNameTV(c.nome)}</span><ServiceBadge s={c.servico} /></div>
                        ))}
                    </div>
                </div>
                {profsAtivos.map(p => {
                    ...
                    return (
                        <div key={p.id} className="glass rounded-[2rem] p-4 flex flex-col border border-yellow-500/20 bg-slate-900/30 overflow-hidden">
                            <h3 className="text-center font-black text-yellow-400 uppercase text-xs md:text-sm tracking-widest mb-3 border-b border-white/5 pb-2 shrink-0">{p.nome} <span className="text-slate-500 mx-1">|</span> CD {p.cadeira} - <span className="text-white text-lg ml-1">{countWaiting}</span><div className={`w-2 h-2 rounded-full mx-auto mt-2 ${p.status === "disponivel" ? "bg-emerald-500 shadow-[0_0_10px_#10b981]" : "bg-orange-500 shadow-[0_0_10px_#f97316]"}`} /></h3>
                            <div className="flex flex-col gap-2 flex-1 overflow-hidden">
                                {displayList.length === 0 && <p className="text-center text-slate-600 font-bold uppercase mt-4 text-[10px]">Livre</p>}
                                {displayList.map((c) => (
                                    <div key={c.id} className={`p-3 md:p-4 rounded-xl border flex flex-col items-center justify-center transition-all relative ${c.isAtendimento ? 'bg-emerald-600/20 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)] flex-1' : 'bg-slate-800/50 border-white/5 shrink-0 pt-6 md:pt-8'}`}>{c.isAtendimento ? (<span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest block mb-2 flex items-center justify-center gap-1"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"/> EM ATENDIMENTO</span>) : (<span className="absolute top-2 right-2 text-[10px] text-slate-400 font-mono font-bold bg-slate-950/50 px-2 py-1 rounded">{c.chegada?.toDate().toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit' })}</span>)}<span className={`font-black block uppercase leading-none mb-1 truncate w-full text-center px-2 ${c.isAtendimento ? 'text-3xl lg:text-4xl text-white' : 'text-xl md:text-2xl text-slate-300'}`}>{formatNameTV(c.nome)}</span><ServiceBadge s={c.servico} /></div>
                                ))}
                            </div>
                        </div>
                    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col text-white">
        <div className="relative z-50">
            {/* Se o barbeiro está no link seguro (isIsolado), NÃO mostra botão de voltar */}
            {!isIsolado && (
                <button onClick={() => { setBarbeiroLogado(null); setModo("selecao"); }} className="absolute top-6 left-6 p-4 bg-slate-900 rounded-3xl text-red-500 hover:bg-red-500 hover:text-white transition-all"><LogOut size={24} /></button>
            )}
        </div>
        <EliteHeader subtitle={`MEU PAINEL - ${barbeiroLogado.nome}`} />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col items-center pb-24">
            ...
            <GlassContainer className="w-full">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4"><h3 className="font-black uppercase tracking-tighter text-xl flex items-center gap-3 select-none text-yellow-500"><Users size={24} /> Meus Clientes ({minhaFilaExclusiva.length})</h3><span className="hidden md:inline text-slate-500">|</span><span className="font-bold text-slate-400 text-sm uppercase tracking-widest">Fila Geral: {totalFilaGeral} aguardando</span></div>
                ...
                <div className="pt-6 border-t border-white/5 mt-4 space-y-3">
                    {minhaFilaExclusiva.length === 0 ? (<p className="text-slate-500 text-xs font-bold uppercase text-center py-4">Nenhum cliente exclusivo aguardando no momento.</p>) : (minhaFilaExclusiva.map(c => (<div key={c.id} className="p-4 bg-slate-900/40 rounded-2xl border border-white/5 flex justify-between items-center hover:border-yellow-500/30 transition-all"><div><span className="font-black text-lg block uppercase text-white">{c.nome}</span><ServiceBadge s={c.servico} /></div><span className="text-xs font-mono font-bold text-slate-400 bg-slate-950 px-3 py-1 rounded-lg">{c.chegada?.toDate().toLocaleTimeString("pt-BR", { hour: '2-digit', minute: '2-digit' })}</span></div>)))}
                </div>
                )}
            </GlassContainer>
            ...
            {/* ÁREAS OCULTAS: FATURAMENTO E DESEMPENHO */}
            <GlassContainer className="w-full bg-slate-900/30">
                <div className="flex items-center justify-between cursor-pointer hover:opacity-80 transition-all group" onClick={() => setShowFatBarbeiro(!showFatBarbeiro)}>
                   <h3 className="font-black uppercase tracking-tighter text-lg flex items-center gap-3 select-none text-emerald-500"><Banknote size={20} /> Faturamento</h3>
                ...
                {showFatBarbeiro && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-6 mt-4 border-t border-white/5">
                        ...
                        <div className="p-6 bg-slate-950/50 rounded-3xl border border-yellow-500/20 overflow-hidden"><span className="block text-[10px] text-yellow-500 font-bold uppercase tracking-widest mb-1">Mês</span><span className="text-2xl font-black block w-full truncate" title={formatCurrency(statsB.mes)}>{formatCurrency(statsB.mes)}</span></div>
                    </div>
                )}
            </GlassContainer>
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
            <div className="glass rounded-[2.5rem] p-10 w-full max-w-md space-y-8 border-emerald-500/30 text-center relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 mt-8">
                ...
                <button onClick={async () => { let duracaoMinutos = 15; if (checkoutAtivo.inicioAtendimento) { const inicio = checkoutAtivo.inicioAtendimento.toMillis ? checkoutAtivo.inicioAtendimento.toMillis() : checkoutAtivo.inicioAtendimento; duracaoMinutos = Math.max(1, Math.floor((Date.now() - inicio) / 60000)); } else if (checkoutAtivo.chegada) { const ch = checkoutAtivo.chegada.toMillis ? checkoutAtivo.chegada.toMillis() : checkoutAtivo.chegada; duracaoMinutos = Math.max(1, Math.floor((Date.now() - ch) / 60000)); } await db.collection("historico_paiva").add({ nome: checkoutAtivo.nome, whatsapp: checkoutAtivo.whatsapp || "", barbeiro: barbeiroLogado.nome, valor: parseFloat(valorInput), duracaoMinutos: duracaoMinutos, isFiel: checkoutAtivo.escolhaDireta || false, dataConclusao: firebase.firestore.Timestamp.now() }); await db.collection("fila_paiva").doc(checkoutAtivo.id).delete(); setCheckoutAtivo(null); addToast("Atendimento Finalizado!", "sucesso"); }} className="p-6 bg-emerald-600 rounded-3xl font-black uppercase text-xs text-white shadow-xl shadow-emerald-900/20 hover:bg-emerald-500 transition-all">CONFIRMAR</button>
              </div>
            </div>
          </div>
        )}
  ...
  if (modo === "gestao_master") {
    ...
    const handleWhatsAppClient = (clientObj, motivo) => {
        ...
        if (phoneRaw.length === 8 || phoneRaw.length === 9) phoneRaw = `5521${phoneRaw}`; else if (phoneRaw.length === 10 || phoneRaw.length === 11) phoneRaw = `55${phoneRaw}`; else if (phoneRaw.length < 8) phoneRaw = `5521${phoneRaw}`;
        window.open(`https://wa.me/${phoneRaw}?text=${encodeURIComponent(texto)}`, '_blank');
    };

    const deletarClienteDB = async (id) => {
        const pin = window.prompt("Atenção: Para excluir este cliente, digite o PIN MASTER (6 dígitos):");
        if (pin !== "123456") return addToast("PIN Incorreto ou cancelado.", "erro");
        await db.collection("clientes_paiva").doc(id).delete();
        addToast("Cliente removido do banco!", "sucesso");
    };

    const limparClientesDB = async () => {
        const pin = window.prompt("PERIGO: Para apagar TODOS os clientes, digite o PIN MASTER:");
        if (pin !== "123456") return addToast("PIN Incorreto.", "erro");
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col text-white">
        <div className="relative z-50">
            <button onClick={() => setModo("selecao")} className="absolute top-6 left-6 bg-slate-900 px-6 py-4 rounded-3xl font-black uppercase text-[10px] tracking-widest border border-white/5 hover:bg-white/5 transition-all"><ArrowLeft size={16} /> Sair</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 flex flex-col items-center">
            ...
            <GlassContainer className="w-full bg-slate-900/30">
                ...
                {showDestaques && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 mt-4 border-t border-white/5">
                        <div className="p-8 bg-gradient-to-br from-yellow-600/20 to-yellow-900/10 rounded-3xl border border-yellow-500/30"><h4 className="text-yellow-500 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Star size={14} /> BARBEIRO ELITE ({advStats.mesPassadoNome})</h4><p className="text-3xl font-black uppercase text-white truncate">{advStats.eliteWinner}</p></div>
                        {advStats.showTri && (<div className="p-8 bg-gradient-to-br from-blue-600/20 to-blue-900/10 rounded-3xl border border-blue-500/30 relative"><h4 className="text-blue-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Users size={14} /> CLIENTE ELITE ({advStats.labelTri})</h4><p className="text-3xl font-black uppercase text-white flex justify-between items-center overflow-hidden"><span className="truncate">{advStats.clientTri.name}</span>{advStats.clientTri.count > 0 && <span className="text-sm font-bold text-slate-400 shrink-0 ml-2">({advStats.clientTri.count} atendimentos)</span>}</p>{advStats.clientTri.count > 0 && (<button onClick={() => handleWhatsAppClient(advStats.clientTri, `cliente elite do ${advStats.labelTri}`)} className="mt-4 w-full bg-green-600 hover:bg-green-500 text-white font-black uppercase text-[10px] tracking-widest p-3 rounded-xl flex items-center justify-center gap-2 transition-all"><MessageCircle size={16} /> Enviar Prêmio via WhatsApp</button>)}</div>)}
                        {advStats.showTri && (<div className="p-8 bg-slate-950/50 rounded-3xl border border-white/10 relative"><h4 className="text-slate-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Trophy size={14} /> BARBEIRO DESTAQUE ({advStats.labelTri})</h4><p className="text-3xl font-black uppercase text-white truncate">{advStats.bTriWinner}</p></div>)}
                        {advStats.showSem && (<div className="p-8 bg-gradient-to-br from-purple-600/20 to-purple-900/10 rounded-3xl border border-purple-500/30 relative col-span-1 md:col-span-2"><h4 className="text-purple-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Trophy size={14} /> BARBEIRO CAMPEÃO DO {advStats.labelSem.toUpperCase()}</h4><p className="text-4xl font-black uppercase text-white neon-purple truncate">{advStats.bSemWinner}</p></div>)}
                        {advStats.showSem && !advStats.showAno && (<div className="p-8 bg-gradient-to-br from-cyan-600/20 to-cyan-900/10 rounded-3xl border border-cyan-500/30 relative col-span-1 md:col-span-2"><h4 className="text-cyan-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Award size={14} /> CLIENTE CAMPEÃO DO {advStats.labelSem.toUpperCase()}</h4><p className="text-3xl font-black uppercase text-white flex justify-between items-center overflow-hidden"><span className="truncate">{advStats.clientSem.name}</span>{advStats.clientSem.count > 0 && <span className="text-sm font-bold text-slate-400 shrink-0 ml-2">({advStats.clientSem.count} atendimentos)</span>}</p>{advStats.clientSem.count > 0 && (<button onClick={() => handleWhatsAppClient(advStats.clientSem, `cliente campeão do ${advStats.labelSem}`)} className="mt-4 w-full bg-green-600 hover:bg-green-500 text-white font-black uppercase text-[10px] tracking-widest p-3 rounded-xl flex items-center justify-center gap-2 transition-all"><MessageCircle size={16} /> Enviar Prêmio via WhatsApp</button>)}</div>)}
                        {advStats.showAno && (<div className="p-8 bg-gradient-to-br from-pink-600/20 to-pink-900/10 rounded-3xl border border-pink-500/30 relative shadow-2xl shadow-pink-500/20 col-span-1 md:col-span-2"><div className="absolute -top-4 -right-4 bg-yellow-500 p-3 rounded-full shadow-lg animate-bounce"><Crown size={24} className="text-black" /></div><h4 className="text-pink-400 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Gift size={14} /> CLIENTE ELITE DO {advStats.labelAno}</h4><p className="text-4xl font-black uppercase text-white flex flex-col"><span className="truncate w-full">{advStats.clientAno.name}</span><span className="text-sm font-bold text-slate-400 mt-1">({advStats.clientAno.count} ATENDIMENTOS NO ANO)</span></p>{advStats.clientAno.count > 0 && (<button onClick={() => handleWhatsAppClient(advStats.clientAno, `cliente do ano de ${new Date().getFullYear()}`)} className="mt-6 w-full bg-yellow-500 hover:bg-yellow-400 text-black font-black uppercase text-[12px] tracking-widest p-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg"><MessageCircle size={18} /> 🎁 ENVIAR SUPER PRÊMIO</button>)}</div>)}
                        {advStats.showAno && (<div className="p-8 bg-gradient-to-br from-yellow-500/20 to-yellow-700/10 rounded-3xl border border-yellow-500/50 relative shadow-2xl shadow-yellow-500/20 col-span-1 md:col-span-2"><div className="absolute -top-4 -right-4 bg-yellow-500 p-3 rounded-full shadow-lg"><Crown size={24} className="text-black" /></div><h4 className="text-yellow-500 font-black uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2"><Trophy size={14} /> BARBEIRO DO {advStats.labelAno}</h4><p className="text-5xl font-black uppercase text-white truncate neon-yellow">{advStats.bAnoWinner}</p></div>)}
                    </div>
                )}
            </GlassContainer>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 mt-4 border-t border-white/5">
                        <div className="space-y-8">
                            <div className="space-y-4 bg-slate-950/50 p-6 rounded-[2rem] border border-white/5">
                                <div className="flex justify-between items-center mb-4"><h4 className="text-yellow-500 font-black uppercase text-xs">{profEditando ? "EDITAR BARBEIRO" : "NOVO BARBEIRO"}</h4>{profEditando && (<button onClick={() => {setProfEditando(null); setNovoProf({ nome: "", matricula: "", cadeira: "" });}} className="text-[10px] text-slate-500 hover:text-white uppercase font-bold">Cancelar</button>)}</div>
                                <input type="text" placeholder="NOME" className="w-full p-4 bg-slate-900 rounded-xl border border-white/5 outline-none focus:border-yellow-500" value={novoProf.nome} onChange={(e) => setNovoProf({ ...novoProf, nome: e.target.value })} />
                                <div className="grid grid-cols-2 gap-4"><input type="text" placeholder="MATRÍCULA" className="w-full p-4 bg-slate-900 rounded-xl border border-white/5 outline-none focus:border-yellow-500" value={novoProf.matricula} onChange={(e) => setNovoProf({ ...novoProf, matricula: e.target.value })} /><input type="number" placeholder="Nº CADEIRA" className="w-full p-4 bg-slate-900 rounded-xl border border-white/5 outline-none focus:border-yellow-500" value={novoProf.cadeira} onChange={(e) => setNovoProf({ ...novoProf, cadeira: e.target.value })} /></div>
                            ...
                            <div className="space-y-4 bg-slate-950/50 p-6 rounded-[2rem] border border-white/5">
                                ...
                                <div className="flex gap-2"><button onClick={() => { window.speechSynthesis.cancel(); const msg = new SpeechSynthesisUtterance("Atenção, Teste de voz ativado."); msg.lang = 'pt-BR'; if (vozSelecionadaUI) { const vozEncontrada = vozesDisponiveis.find(v => v.name === vozSelecionadaUI); if (vozEncontrada) msg.voice = vozEncontrada; } window.speechSynthesis.speak(msg); }} className="flex-1 p-3 rounded-xl font-black text-slate-300 bg-slate-800 hover:bg-slate-700 transition-all text-xs">TESTAR VOZ</button><button onClick={async () => { await db.collection("configuracoes_paiva").doc("geral").set({ vozTV: vozSelecionadaUI }, { merge: true }); addToast("Voz da TV salva!", "sucesso"); }} className="flex-1 p-3 rounded-xl font-black text-black bg-emerald-500 hover:bg-emerald-400 transition-all text-xs">SALVAR NA NUVEM</button></div>
                            </div>
                        </div>
                        <div className="space-y-3 h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                            {sortedProfsMaster.map((p) => (<div key={p.id} className={`p-4 rounded-2xl flex justify-between items-center border transition-all ${profEditando === p.id ? 'bg-blue-900/20 border-blue-500/50' : 'bg-slate-950/50 border-white/5'}`}><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center font-black text-slate-500">{p.cadeira || "-"}</div><div><span className="font-black uppercase text-sm block text-white">{p.nome}</span><span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono">PIN: {p.matricula}</span></div></div><div className="flex gap-2"><button onClick={() => { const link = `${window.location.origin}?barbeiro=${p.matricula}`; navigator.clipboard.writeText(link); addToast("Link copiado!", "sucesso"); }} className="p-2.5 bg-slate-800 rounded-lg text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all" title="Copiar Link"><Link size={16} /></button><button onClick={() => { setProfEditando(p.id); setNovoProf({ nome: p.nome, matricula: p.matricula, cadeira: p.cadeira || "" }); }} className="p-2.5 bg-slate-800 rounded-lg text-slate-400 hover:text-yellow-400 hover:bg-yellow-500/10 transition-all" title="Editar"><Edit2 size={16} /></button><button onClick={() => { if(window.confirm(`Deseja demitir/excluir ${p.nome}?`)) { db.collection("profissionais").doc(p.id).delete() } }} className="p-2.5 bg-slate-800 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-all" title="Excluir"><Trash2 size={16} /></button></div></div>))}
                        </div>
                    </div>
                )}
            </GlassContainer>
            ...
            <GlassContainer className="w-full bg-slate-900/30">
                ...
                <div className="flex items-center gap-4">{showClientesDB && (<button onClick={(e) => { e.stopPropagation(); limparClientesDB(); }} className="bg-red-600/10 text-red-500 border border-red-500/20 px-4 py-2 rounded-2xl font-black uppercase text-[9px] flex items-center gap-2 hover:bg-red-600 hover:text-white transition-all"><Trash2 size={14} /> Apagar Todos</button>)}{showClientesDB ? <ChevronUp className="text-slate-500 group-hover:text-white transition-all" /> : <ChevronDown className="text-slate-500 group-hover:text-white transition-all" />}</div>
                </div>
                {showClientesDB && (<div className="pt-6 border-t border-white/5 mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">{clientesDB.length === 0 && <p className="text-slate-500 text-sm font-bold uppercase col-span-2">Nenhum cliente salvo no banco de dados ainda.</p>}{clientesDB.map(c => (<div key={c.id} className="p-4 bg-slate-950/50 border border-white/5 rounded-2xl flex justify-between items-center hover:border-blue-500/30 transition-all"><div><span className="block font-black text-white uppercase">{c.nome}</span><span className="text-xs font-mono text-slate-400 block mt-1">CPF: {c.cpf} | Tel: {c.whatsapp}</span></div><button onClick={() => deletarClienteDB(c.id)} className="p-3 bg-red-900/20 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all" title="Apagar Cliente"><Trash2 size={16}/></button></div>))}</div>)}
            </GlassContainer>
            ...
            <GlassContainer className="w-full bg-slate-900/30">
                ...
                <div className="flex items-center gap-4">{showHistorico && (<button onClick={(e) => { e.stopPropagation(); limparHistoricoCompleto(); }} className="bg-red-600/10 text-red-500 border border-red-500/20 px-4 py-2 rounded-2xl font-black uppercase text-[9px] flex items-center gap-2 hover:bg-red-600 hover:text-white transition-all"><Trash2 size={14} /> Apagar Histórico</button>)}{showHistorico ? <ChevronUp className="text-slate-500 group-hover:text-white transition-all" /> : <ChevronDown className="text-slate-500 group-hover:text-white transition-all" />}</div>
                </div>
                {showHistorico && (<div className="overflow-x-auto pt-6 border-t border-white/5 mt-4"><table className="w-full text-left"><thead className="text-[10px] uppercase font-black text-slate-700 border-b border-white/5"><tr><th className="pb-6 px-4">DATA</th><th className="pb-6 px-4">Cliente</th><th className="pb-6 px-4">Contato (Invisível TV)</th><th className="pb-6 px-4 text-center">Barbeiro</th><th className="pb-6 px-4 text-center">Valor</th><th className="pb-6 px-4 text-right">Hora</th></tr></thead><tbody className="text-xs">{historicoAtendimentos.map((h) => (<tr key={h.id} className="border-b border-white/5 hover:bg-white/5 transition-all"><td className="py-6 px-4 font-black text-slate-400 font-mono">{h.dataConclusao?.toDate().toLocaleDateString("pt-BR")}</td><td className="py-6 px-4 font-black uppercase text-slate-300 tracking-tighter text-lg flex items-center gap-2">{h.nome} {h.isFiel && <Heart size={12} className="text-pink-500" title="Cliente escolheu este barbeiro" />}</td><td className="py-6 px-4 text-slate-500 font-mono tracking-widest">{h.whatsapp || "Sem Registro"}</td><td className="py-6 px-4 text-center"><span className="px-4 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-full text-[10px] font-black uppercase text-yellow-500"><Scissors size={10} className="inline mr-1" /> {h.barbeiro}</span></td><td className="py-6 px-4 text-center font-black text-emerald-400">{formatCurrency(h.valor || 0)}</td><td className="py-6 px-4 text-right text-slate-500 font-mono">{h.dataConclusao?.toDate().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</td></tr>))}</tbody></table></div>)}
            </GlassContainer>
            </div>
        </div>
        <ISDSignature />
